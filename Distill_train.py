...
from utils.distill_fun import getMask,compute_mask_loss
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    teacherModel = attempt_load(opt.teacher_weights, map_location=device)  # load FP32 model
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%10s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls','lsoft', 'total', 'targets', 'img_size'))
        if rank in [-1, 0]:
        ...
        for i, (imgs, targets, paths, shapes) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                feat_1,feat_2,feat_3 = pred
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                anchors = compute_loss.anchors.cpu().numpy()
                mask_1 = getMask(batch_size, targets,imgs.shape[2:],  feat_1,anchors[0,:,:])
                mask_2 = getMask(batch_size, targets,imgs.shape[2:],  feat_2,anchors[1,:,:])
                mask_3 = getMask(batch_size, targets,imgs.shape[2:],  feat_3,anchors[2,:,:])
